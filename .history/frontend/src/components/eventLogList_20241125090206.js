  ...
  const fetchEventLogs = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        // Remove duplicates based on some unique property, e.g., _id
        const uniqueLogs = Array.from(new Set(data.map(log => log._id)))
          .map(id => data.find(log => log._id === id));
        setEventLogs(uniqueLogs);
      } else {
        setEventLogs([]);
      }
    } catch (error) {
