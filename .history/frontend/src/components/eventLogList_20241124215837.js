...
import { Box, Button, TextField, Typography, DataGrid } from '@mui/material';
...
const EventLogList = () => {
  ...
  const fetchEventLogs = async () => {
    try {
      ...
      setEventLogs(Array.isArray(data.logs) ? data.logs : []); // Ensure it's an array
    } catch (error) {
      console.error('Error fetching event logs:', error);
      setEventLogs([]); // Reset logs on error
    }
  };
